...
/********************* VARIABLES *******************************/
	...
	DECLARE @ObjectName varchar(256) = 'Collector.usp_Maintenance_Backups_DeleteFolderContents_JobCreate';
	DECLARE @JobName varchar(256) = '$(Database_Name).Collector.Maintenance_Backups_DeleteFolderContents';
...
-- Description:	SP that will create job
-- =============================================
...
BEGIN TRY;
	...
	go
	exec [Collector].[usp_Maintenance_Backups_DeleteFolderContents]'', 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N''' + @JobName + ''', 
			@enabled=1, 
			@freq_type=4, 
			@freq_interval=1, 
			@freq_subday_type=1, 
