...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N''' + @ObjectName + ''', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N''Reporting on all failed SQL Jobs. Run every 1 hr'', 
		@category_name=N''Database Maintenance'', 
		@owner_login_name=N''sa'', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Insert of Data]    Script Date: 9/24/2014 8:19:34 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N''Insert of Data'', 
		@step_id=1, 
...
go
exec [Reporting].[usp_FailedSQLJobsHistory_Insert]'', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N''CollectorSchedule_Every_30min'', 
		...
		@schedule_uid=N''44149a24-c866-4aa9-987e-697232287444''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
